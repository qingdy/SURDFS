...
public:
    void Run(CThread * thread, void * arg)
    {
        ...
        for (int i = 0; i < 5; ++i) {
            client->MakeDirectory(src + Int32ToString(*((int64_t*)arg)) + dname[i]);
            client->GetFileInfo(src + Int32ToString(*((int64_t*)arg)) + dname[i]);
        }

        for (int32_t k = 0; k < filenum -5; ++k) {
            client->Create(src + Int32ToString(*((int64_t*)arg)) + dname[5 * k / (filenum - 5)] + "_" + Int32ToString(k), 1, 3);
        }
