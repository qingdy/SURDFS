...
typedef bladestore::btree::KeyBtree<TestKey, int32_t> StringBtree;
...
void test_insert(int32_t cnt)
{
  ...
  for(int32_t i = 0; i < cnt; i++)
  {
    ...
    if (ret != bladestore::btree::ERROR_CODE_OK)
    {
      failure ++;
    }
  }
...
{
  bladestore::btree::BtreeWriteHandle handle;
  ...
  assert(bladestore::btree::ERROR_CODE_OK == ret);
  for(int32_t i = 0; i < cnt; i++)
  {
    if (get_next_id(j)) break;
    sprintf(buffer, "%08X", j);
...
{
  bladestore::btree::BtreeReadHandle handle;
  ...
  while(bladestore::btree::ERROR_CODE_OK == btree->get_next(handle, value))
  {
    success ++;
  }
  if (success == cnt)
  ...
  success = 0;
  bladestore::btree::BtreeReadHandle handle1;
  btree->get_read_handle(handle1);
  btree->set_key_range(handle1, btree->get_max_key(), 1, btree->get_min_key(), 1);
  while(bladestore::btree::ERROR_CODE_OK == btree->get_next(handle1, value))
  {
    success ++;
  }
  if (success == cnt - 2)
