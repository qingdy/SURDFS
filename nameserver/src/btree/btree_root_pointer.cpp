...
void BtreeRootPointer::set_read_only(BtreeAlloc *key_allocator, bool remove)
{
	...
	while ((node = reinterpret_cast<BtreeNode*>(node_list_.node_list_next(list))) != NULL)
	{
		if (node->is_deleted())
		{
			if (remove && node_allocator_)
			{
				if (key_allocator) 
				{
					node->dec_key_refcount(key_allocator);
				}
	...
	BLADE_BTREE_CHECK_TRUE(list.pos == list.node_count, "cnt: %d %d", list.pos, list.node_count);
	new_node_create_ = 0;
}
...
{
	BLADE_BTREE_CHECK_TRUE(node_allocator_, "node_allocator is null, this=%p", this);
...
void BtreeRootPointer::Destroy()
{
	...
	if (na) 
	{
		na->release(reinterpret_cast<char*>(this));
	}
}
...
int32_t BtreeRootPointer::release_node_list(int64_t sequence, BtreeAlloc *key_has_ref)
{
	...
	BLADE_BTREE_CHECK_TRUE(list.pos == list.node_count,"cnt: %d %d", list.pos, list.node_count);
	BLADE_BTREE_CHECK_TRUE(ret == node_list_.get_copy_node_count(), "ret: %d %d", ret, node_list_.get_copy_node_count());
...
BtreeNode *BtreeRootPointer::new_node(int32_t count)
{
	...
	if (node)
	{
		...
		BLADE_BTREE_CHECK_TRUE(0 == node->is_deleted(), "deleted %d", node->is_deleted());
		node_list_.add_node_list(node_allocator_, 1, node);
		new_node_create_ = 1;
	}
	return node;
...
BtreeNode *BtreeRootPointer::copy_node(BtreeNode *node, BtreeAlloc *key_has_ref)
{
	...
	if (node)
	{
		if (node_allocator_)
		{
			...
			BLADE_BTREE_CHECK_TRUE(new_node, "node is null.");
		}
