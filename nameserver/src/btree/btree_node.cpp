...
{
	BLADE_BTREE_CHECK_TRUE(max_size > 0, "max_size: %d", max_size);
...
{
	BLADE_BTREE_CHECK_TRUE(0 == read_only_, "read_only: %d", read_only_);
	BLADE_BTREE_CHECK_TRUE(pos >= 0 && pos <= size_, "pos: %d, size: %d", pos, size_);
	BLADE_BTREE_CHECK_TRUE(size_ >= 0 && size_ < max_size_ && max_size_ == CONST_NODE_OBJECT_COUNT, "size: %d, max_size: %d, cnt: %d", size_, max_size_, CONST_NODE_OBJECT_COUNT);
	...
	{
		BLADE_BTREE_CHECK_TRUE(0, "smaller than the left key, pos=%d, flag=%ld", pos, f);
	}
	if (compr && pos + 1 < size_ && (f = (*compr)(key, pairs_[pos+1].key_)) >= 0)
	{
		BLADE_BTREE_CHECK_TRUE(0, "bigger than the right key, pos=%d, flag=%ld", pos, f);
	}
#endif
...
{
	BLADE_BTREE_CHECK_TRUE(0 == read_only_ && pos >= 0 && pos < size_, "read_only: %d, pos: %d, size: %d", read_only_, pos, size_);
	BLADE_BTREE_CHECK_TRUE(max_size_ == CONST_NODE_OBJECT_COUNT, "max_size: %d", max_size_);
...
{
	flag = -1;
	BLADE_BTREE_CHECK_FALSE(size_ < 0 || max_size_ != CONST_NODE_OBJECT_COUNT || size_ > max_size_ , "size: %d max_size: %d", size_, max_size_);
...
{
	BLADE_BTREE_CHECK_TRUE(next_node, "next_node is null.");
	if (next_node)
	{
		BLADE_BTREE_CHECK_TRUE((0 == read_only_ && 0 == next_node->read_only_ &&
...
int32_t BtreeNode::merge_pair_in(BtreeNode *next_node)
{
	...
	if (next_node)
	{
		...
		printf("360 %d\n", size);
		BLADE_BTREE_CHECK_TRUE((0 == read_only_ && ((size + size_) <= max_size_)),
					"read_only: %d, size: %d, size_:%d, max_size: %d",
					read_only_, size, size_, max_size_);
		BLADE_BTREE_CHECK_TRUE(max_size_ == CONST_NODE_OBJECT_COUNT, "max_size: %d", max_size_);

		if (0 == read_only_ && size >= 0 && ((size + size_) <= max_size_) && max_size_ == CONST_NODE_OBJECT_COUNT)
		{
...
{
	BLADE_BTREE_CHECK_TRUE(dst >= 0 && dst < max_size_, "dst: %d, max_size: %d", dst, max_size_);
	BLADE_BTREE_CHECK_TRUE(src >= 0 && src < max_size_, "src: %d, max_size: %d", src, max_size_);
	BLADE_BTREE_CHECK_TRUE(n > 0 && n + src <= max_size_, "n: %d src: %d, max_size: %d", n, src, max_size_);
	BLADE_BTREE_CHECK_TRUE(max_size_ == CONST_NODE_OBJECT_COUNT, "max_size: %d", max_size_);
	BLADE_BTREE_CHECK_TRUE(dst < src, "dst: %d, src: %d", dst, src);
...
void BtreeNode::pair_move_backward(int16_t dst, int16_t src, int32_t n)
{
	BLADE_BTREE_CHECK_TRUE(dst >= 0 && dst < max_size_, "dst: %d, max_size: %d", dst, max_size_);
	BLADE_BTREE_CHECK_TRUE(src >= 0 && src < max_size_, "src: %d, max_size: %d", src, max_size_);
	BLADE_BTREE_CHECK_TRUE(n > 0 && n + dst <= max_size_, "n: %d dst: %d, max_size: %d", n, dst, max_size_);
...
void BtreeNode::inc_key_refcount()
{
	if (is_leaf_)
	{
		BLADE_BTREE_CHECK_TRUE(size_ <= max_size_ && max_size_ == CONST_NODE_OBJECT_COUNT, "size: %d, max_size: %d", size_, max_size_);

		int32_t *addr;
		BLADE_BTREE_CHECK_TRUE(read_only_ == 0, "is read only.");
		for (int32_t i = 0; i < size_; i++)
		{
			addr = reinterpret_cast<int32_t*>(pairs_[i].key_);
			BLADE_BTREE_CHECK_TRUE(addr, "addr is null.");
			if (addr)
			{
				BLADE_BTREE_CHECK_TRUE((*addr) >= 0, "addr:%p pos:%d value:%d", addr, i, *addr);
				(*addr) ++;
			}
		}
	}
