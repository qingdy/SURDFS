...
#ifndef BLADESTORE_BTREE_KEY_BTREE_H
#define BLADESTORE_BTREE_KEY_BTREE_H
...
public:
	 ...
	 */
	void set_key_range(BtreeReadHandle &handle, const K *from_key, int32_t from_exclude, const K *to_key, int32_t to_exclude);
 ...
 */
template<class K, class V>
KeyBtree<K, V>::KeyBtree(int32_t key_size, BtreeAlloc *key_allocator,BtreeAlloc *node_allocator) : BtreeBase(node_allocator)
{
	tree_count_ = 1;
	// 引用计数一个int32_t
...
int32_t KeyBtree<K, V>::Remove(const K &key)
{
	...
	if (ERROR_CODE_OK == ret)
	{
		...
		if(NULL != handle.get_old_value())
		{
			delete (V)(handle.get_old_value());
		}
	}
	return ret;
}
 ...
 */
template<class K, class V>
void KeyBtree<K, V>::set_key_range(BtreeReadHandle &handle, const K *from_key, int32_t from_exclude, const K *to_key, int32_t to_exclude)
{
	// 设置from_key
