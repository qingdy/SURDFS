	...
	, void *node)
{
	...
	BLADE_BTREE_CHECK_TRUE(node_allocator, "node is null.");
	BLADE_BTREE_CHECK_TRUE(copy_node_count_ >= 0, "copy_node_count_: %d", copy_node_count_);
	...
	if (NULL != node && copy_node_count_ >= 0 && node_allocator)
	{
		...
		if (unique && bf.exist(node))
		{
			...
			while ((tn = node_list_next(tlist)) != NULL)
			{
				if (tn == node)
				{
					BLADE_BTREE_CHECK_TRUE(0, "node is repeat.");
					abort();
				}
			}
		}
		...
		else
		{
			...
			if (NULL == list)
			{
				...
				BLADE_BTREE_CHECK_TRUE(list, "list is null.");
				if (NULL != copy_list_current_)
				{
...
void *BtreeArrayList::node_list_next(BtreeNodeList &list)
{
	...
	else if (list.pos >= 0 && NULL != list.start && NULL != list.current && list.current > list.start)
	{
...
{
	BLADE_BTREE_CHECK_TRUE(node_allocator, "node_allocator is null.");
	void **pdata_next;
