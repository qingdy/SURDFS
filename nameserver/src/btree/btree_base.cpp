...
namespace btree
{
	...
	int32_t BtreeBase::get_read_handle(BtreeBaseHandle &handle)
	{
		...
		BLADE_BTREE_CHECK_FALSE(handle.root_pointer_, "handle.root_pointer_ is not null.");
	...
	{
		BLADE_BTREE_CHECK_TRUE(node_allocator_, "node_allocator_ is null.");
		...
		BLADE_BTREE_CHECK_TRUE(proot, "proot is null.");
		int32_t ret = ERROR_CODE_FAIL;
		if (NULL == proot)
		{
			handle.End();
		...
		else
		{
			...
			// 取到只读的root
			BLADE_BTREE_CHECK_TRUE(root_pointer_, "root_pointer is null.");
	...
	{
		BLADE_BTREE_CHECK_FALSE(NULL == handle.root_pointer_ || tree_id < 0
		...
		if (ERROR_CODE_OK == get_children(proot, tree_id, key, param))
		{
			...
			BLADE_BTREE_CHECK_TRUE(level >= 0 && level < CONST_MAX_DEPTH, "level: %d", level);
			if (0 == param.found_ && level >= 0 && level < CONST_MAX_DEPTH)
			{
				pair = param.node_[level]->get_pair(param.node_pos_[level]);
				BLADE_BTREE_CHECK_TRUE(pair, "pair is null, level:%d, pos:%d, node:%p, size:%d, tree_id:%d key:%p",
						level, param.node_pos_[level], param.node_[level],
						param.node_[level]->get_size(), tree_id, key);
			}
		}
		//BLADE_BTREE_CHECK_TRUE(pair, "pair is null, key=%p, tree_id=%d, proot=%p",
		//  key, tree_id, proot);
	...
	{
		BLADE_BTREE_CHECK_FALSE(NULL == handle.root_pointer_ || handle.tree_id_ < 0
		...
		if (0 == param->node_length_)
		{
			...
			BLADE_BTREE_CHECK_TRUE(to_param->node_length_ == param->node_length_,
		...
		if (0 == handle.is_eof_ && level >= 0 && level < CONST_MAX_DEPTH)
		{
					...
					to_param->node_pos_[level] == param->node_pos_[level])
			{
				...
				if (0 == handle.to_exclude_)
				{
					pair = param->node_[level]->get_pair(param->node_pos_[level]);
					handle.to_exclude_ = 1;
					BLADE_BTREE_CHECK_TRUE(pair, "level: %d, pos: %d, node: %p, size: %d", level,
	...
	{
		int32_t is_eof = 0;
		BLADE_BTREE_CHECK_TRUE(param, "param is null.");
		if (NULL != param)
		{
			int32_t level = param->node_length_ - 1;
			BLADE_BTREE_CHECK_TRUE(level >= 0 && level < CONST_MAX_DEPTH, "level: %d", level);
			if (level >= 0 && level < CONST_MAX_DEPTH)
			{
				...
				while (0 == is_eof && level < param->node_length_ - 1 && level >= 0 && level < CONST_MAX_DEPTH - 1)
				{
					...
					BLADE_BTREE_CHECK_TRUE(parent, "parent is null.");
					if (NULL != parent)
					{
						pair = parent->get_pair(param->node_pos_[level]);
						BLADE_BTREE_CHECK_TRUE(pair, "pair is null.");
						if (NULL == pair)
						{
							is_eof = 1;
							break;
						...
						BLADE_BTREE_CHECK_TRUE(parent->is_leaf() == 0, "parent isn't leaf node.");
						param->node_[level+1] = reinterpret_cast<BtreeNode*>(pair->value_);
						BLADE_BTREE_CHECK_TRUE(param->node_[level+1],
						...
						if (param->node_pos_[level+1] < 0)
						{
							...
							BLADE_BTREE_CHECK_TRUE(param->node_pos_[level+1] >= 0,
	...
	{
		BLADE_BTREE_CHECK_TRUE(proot, "proot is null.");
		BLADE_BTREE_CHECK_TRUE(tree_id >= 0 && tree_id < CONST_MAX_BTREE_COUNT, "tree_id: %d", tree_id);
		...
		if (NULL != proot && tree_id >= 0 && tree_id < CONST_MAX_BTREE_COUNT)
		{
			...
			for(int32_t i = 0; NULL != root && i < depth && i < CONST_MAX_DEPTH; i++)
			{
				...
				if (CONST_NODE_OBJECT_COUNT == pos) 
				{
					break;
				}
				...
				BLADE_BTREE_CHECK_TRUE(root->is_leaf() == 0, "root isn't leaf node.");
	...
	int32_t BtreeBase::put_pair(BtreeWriteHandle &handle, const char *key, const char *value, const bool overwrite)
	{
		...
		BLADE_BTREE_CHECK_FALSE((NULL == handle.root_pointer_ || NULL != handle.next_handle_),
		...
		if (NULL != handle.root_pointer_ && NULL == handle.next_handle_)
		{
			...
			if (0 == proot->node_count_)
			{
				...
				for(int32_t id = 0; id < tree_count_; id ++)
				{
					...
					BLADE_BTREE_CHECK_TRUE(proot->root_[id], "proot->root_[%d] is null", id);
					if (NULL != proot->root_[id])
					{
						...
						BLADE_BTREE_CHECK_TRUE(ERROR_CODE_OK == ret, "ret:%d, key:%p, value:%p, id:%d",
								ret,  key, value, id);
			...
			else
			{
				...
				for(int32_t id = 0; id < tree_count_; id ++)
				{
					...
					BLADE_BTREE_CHECK_TRUE(ERROR_CODE_OK == ret, "ret:%d, id:%d key:%p",
							ret, id, key);
				...
				if (ERROR_CODE_OK == ret)
				{
					...
					for (int32_t id = 0; id < tree_count_; id ++)
					{
						...
						BLADE_BTREE_CHECK_TRUE(ERROR_CODE_OK == ret, "ret:%d, id=%d", ret, id);
						if (ERROR_CODE_OK != ret) break;

						ret = add_pair_to_node(handle, params[id], params[id].node_length_ - 1, key, value);
						BLADE_BTREE_CHECK_TRUE(ERROR_CODE_OK == ret, "ret:%d, id:%d len:%d, key:%p, value:%p",
				...
				else if ((true == overwrite) && (ERROR_CODE_KEY_REPEAT == ret))
				{
					...
					if (index >= 0 && index < CONST_MAX_DEPTH)
					{
						...
						BLADE_BTREE_CHECK_TRUE(oldpair, "oldpair is null, index:%d, pos:%d",
					...
					if (NULL != oldpair)
					{
						...
						{
							ret = copy_node_for_write(handle, params[id]);
							BLADE_BTREE_CHECK_TRUE(ERROR_CODE_OK == ret, "ret:%d, id:5d", ret, id);
							if (ERROR_CODE_OK != ret) break;
							...
							ret = params[id].node_[index]->set_pair(params[id].node_pos_[index],
									...
									key_compare_[id]);
							BLADE_BTREE_CHECK_TRUE(ERROR_CODE_OK == ret, "ret:%d, id:%d, index:%d key:%p value:%p",
				...
				for(int32_t id = 0; ERROR_CODE_OK == ret && id < tree_count_; id ++)
				{
					if (ERROR_CODE_OK == get_children(proot, id, check_key, params[id]))
					{
						for(int32_t i = 0; i < params[id].node_length_ - 1; i++)
						{
							...
							BLADE_BTREE_CHECK_TRUE(pair && pair->value_ ==
							...
							if (pair && pair->value_)
							{
								...
								BLADE_BTREE_CHECK_TRUE(fpair && pair && pair->key_ == fpair->key_,
	...
	int32_t BtreeBase::remove_pair(BtreeWriteHandle &handle, const int32_t tree_id, const char *key)
	{
		...
		handle.old_value_ = NULL;
		BLADE_BTREE_CHECK_TRUE(proot, "proot is null.");
		BLADE_BTREE_CHECK_TRUE(tree_id >= 0 && tree_id < CONST_MAX_BTREE_COUNT,
		...
		if (NULL != proot && tree_id >= 0 && tree_id < CONST_MAX_BTREE_COUNT)
		{
			...
			if (proot->root_[0] != NULL)
			{
				...
				BLADE_BTREE_CHECK_TRUE(ERROR_CODE_OK == ret, "ret: %d", ret);
						...
						&& level >= 0 && level < CONST_MAX_DEPTH)
				{
					...
					for(int32_t id = 0; ERROR_CODE_OK == ret && id < tree_count_; id ++)
					{
						...
						{
							BLADE_BTREE_CHECK_TRUE(proot->root_[id]->is_deleted() == 0, "root is deleted");
						}
					}
				}
				else
	...
	int32_t BtreeBase::copy_node_for_write(BtreeWriteHandle &handle, BtreeWriteParam &param)
	{
		...
		{
			BLADE_BTREE_CHECK_TRUE(param.node_[i], "param.node_[%d] is null.", i);
			...
			BLADE_BTREE_CHECK_TRUE(handle.prev_root_pointer_, "handle.prev_root_pointer_ is null.");
			BLADE_BTREE_CHECK_TRUE(handle.root_pointer_, "handle.root_pointer_ is null.");
	...
	{
		BLADE_BTREE_CHECK_TRUE(handle.root_pointer_, "handle.root_pointer_ is null.");
		BLADE_BTREE_CHECK_FALSE(level < 0 || level >= CONST_MAX_DEPTH, "level: %d", level);
		BLADE_BTREE_CHECK_FALSE(param.node_length_ < 0 || param.node_length_ >= CONST_MAX_DEPTH, "level: %d", level);
		...
		BLADE_BTREE_CHECK_TRUE(node, "node is null.");
		// 叶节点没满, 直接插入
		...
		else if (node)
		{
			...
			else if (next_node && is_no_full)
			{
				...
				BLADE_BTREE_CHECK_TRUE(move_size >= 1 && move_size <= CONST_NODE_OBJECT_COUNT - next_node->get_size(),
			...
			else
			{
				...
				BLADE_BTREE_CHECK_TRUE(new_node, "new_node is null.");
				if (NULL != new_node)
				{
					proot->node_count_ ++;
			...
			BtreeWriteParam& param, int32_t level, int32_t next_pos)
	{
		BLADE_BTREE_CHECK_TRUE(handle.root_pointer_, "handle.root_pointer_ is null.");
		BLADE_BTREE_CHECK_FALSE(level < 0 || level >= CONST_MAX_DEPTH, "level: %d", level);
		BLADE_BTREE_CHECK_FALSE(next_pos < 0 || next_pos > 1, "next_pos: %d", next_pos);
		BLADE_BTREE_CHECK_FALSE(param.node_length_ < 0 || param.node_length_ >= CONST_MAX_DEPTH, "level: %d", level);
		if (level < 0 || level >= CONST_MAX_DEPTH || NULL == handle.root_pointer_ ||
		...
		if (NULL != node)
		{
			...
			if (0 == level)
			{
				...
				if (1 == node->get_size() && param.node_length_ > 1)
				{
					...
					BLADE_BTREE_CHECK_TRUE(first_key, "first_key is null.");
	...
	{
		BLADE_BTREE_CHECK_FALSE(level < 0 || level >= CONST_MAX_DEPTH, "level: %d", level);
		BLADE_BTREE_CHECK_FALSE(param.tree_id < 0 || param.tree_id > CONST_MAX_BTREE_COUNT,
	...
	void BtreeBase::update_parent_first_key(BtreeWriteHandle &handle, BtreeWriteParam& param, int32_t level)
	{
		...
		if (NULL != node && NULL != parent)
		{
			...
			if (key_allocator_ && first_key)
			{
				...
				BLADE_BTREE_CHECK_TRUE(old_key, "old_key is null.");
	...
	int32_t BtreeBase::callback_done(void *data)
	{
		...
		BLADE_BTREE_CHECK_TRUE(handle, "handle is null.");
		BLADE_BTREE_CHECK_TRUE(handle->root_pointer_, "handle->root_pointer is null.");
		BLADE_BTREE_CHECK_TRUE(node_allocator_, "node_allocator_ is null.");
	...
	{
		BLADE_BTREE_CHECK_TRUE(root, "root is null.");
		if (NULL == root)
			return;
	...
	void BtreeBase::Clear()
	{
		...
		BtreeRootPointer *proot = handle.root_pointer_;
		BLADE_BTREE_CHECK_TRUE(proot, "root is null.");
	...
	int32_t BtreeBase::get_alloc_memory()
	{
		...
		if (node_allocator_) 
		{
			ret += node_allocator_->get_malloc_count();
		}
