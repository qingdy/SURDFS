...
void BtreeWriteHandle::End()
{
	...
	if (NULL != next_handle_)
	{
		...
		BLADE_BTREE_CHECK_TRUE(node_allocator_, "node_allocator is null.");
		if (node_allocator_)
		{
	...
	if (key_allocator_ && ERROR_CODE_OK == error_)
	{
		...
		while ((old_addr = reinterpret_cast<int32_t*>(new_key_list_.node_list_next(list))) != NULL)
		{
			...
			{
				BLADE_BTREE_CHECK_TRUE((*old_addr) >= 0, "old_addr: %d", (*old_addr));
				(*old_addr) ++;
			}
		}
		...
		{
			BLADE_BTREE_CHECK_TRUE(old_addr, "old_addr is null.");
			if (parent_)
			{
				parent_->add_key_to_list(reinterpret_cast<char*>(old_addr), -1);
			}
...
BtreeWriteHandle *BtreeWriteHandle::get_write_handle()
{
	...
	if (!prev_root_pointer_)
	{
		if (node_allocator_)
		{
			...
			BLADE_BTREE_CHECK_TRUE(prev_root_pointer_, "prev_root_pointer_ is null.");
			if (prev_root_pointer_) 
				prev_root_pointer_->Init(node_allocator_);
		}
		ppr_create_ = 2;
	}
	...
	if (prev_root_pointer_ && NULL == next_handle_ && NULL != root_pointer_)
	{
		...
		BLADE_BTREE_CHECK_TRUE(handle && proot && prev_proot, "handle: %p, proot: %p, prev_proot: %p", handle, proot, prev_proot);
...
int BtreeWriteHandle::callback_done(void *data)
{
	...
	BLADE_BTREE_CHECK_TRUE(handle && root_pointer_ && prev_root_pointer_, "handle: %p, root_pointer_: %p, prev_root_pointer: %p", handle, root_pointer_, prev_root_pointer_);

	if (handle)
	{
		BLADE_BTREE_CHECK_TRUE(handle->root_pointer_ && handle->prev_root_pointer_, "root_pointer_: %p, prev_root_pointer: %p", handle->root_pointer_, handle->prev_root_pointer_);
	}

	if (handle && root_pointer_ && handle->root_pointer_ && prev_root_pointer_ && handle->prev_root_pointer_)
	{
		...
		{
			if (0 == node->is_deleted()) 
			{
				root_pointer_->add_node_list(node);
			}
		}
