...
BtreeDefaultAlloc::BtreeDefaultAlloc() : BtreeAlloc()
{
	...
	malloc_count_ = 0;
	init(sizeof(int64_t));
	BLADE_BTREE_CHECK_TRUE(free_count_ >= 0, "xxx");
}
...
{
	BLADE_BTREE_CHECK_TRUE(size >= static_cast<int32_t>(sizeof(int64_t)), "ret:%d", size);
	destroy();
	size_ = size;
...
{
	char *pret = NULL;
#ifdef BLADE_BASE_BTREE_USE_SYS_MALLOC
	pret = (char*)malloc(size_);
	//BLADE_BTREE_CHECK_TRUE(0, "ALLOC: %p", pret);
#else
	...
	if (free_list_ != NULL)
	{
		...
		BLADE_BTREE_CHECK_TRUE(free_count_ > 0, "free_count_: %d", free_count_);
		free_count_ --;
	}
	...
	else
	{
		...
		malloc_count_ ++;
		// 分配出1m空间
		int32_t n = BtreeDefaultAlloc::MALLOC_SIZE / size_;
		n = (n < 1 ? 1 : n) * size_ + sizeof(struct MemBuffer);
		char *pdata = reinterpret_cast<char*>(malloc(n));
		BLADE_BTREE_CHECK_TRUE(pdata, "pdata is null.");
		if (NULL != pdata)
		{
...
#endif
	BLADE_BTREE_CHECK_TRUE(pret, "pret is null.");
...
{
	BLADE_BTREE_CHECK_TRUE(pdata, "pret is null.");
	if (pdata != NULL)
	{
		BLADE_BTREE_CHECK_TRUE(use_count_ > 0, "use_count_: %d", use_count_);
		use_count_  --;
		free_count_ ++;

#ifdef BLADE_BASE_BTREE_USE_SYS_MALLOC
		//BLADE_BTREE_CHECK_TRUE(0, "FREE: %p", pdata);
		free((void*)pdata);
#else
...
void BtreeDefaultAlloc::destroy()
{
	if (buffer_ != NULL)
	{
		...
		while (mb)
		{
			BLADE_BTREE_CHECK_TRUE(mb, "mb is null");
			next_mb = mb->next;
